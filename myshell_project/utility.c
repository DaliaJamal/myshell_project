...
int Command_exec (char **args, const Redirect *Inputs, const Redirect *Outputs, int *states) {
	...
	if (!strcmp(args[0], "cd"))	// "cd " command
		...
		if (output_num == 0)
			Command_clear();	// In Command_clear() ｼ憩xecute clear
...
int Command_help (char **args, const Redirect *Outputs, int *states) {
	...
	readme = fopen(getenv("readme_path"), "r"); // while execute myshell ｼ径dd readme_path into the environment variables in 							     // initialization phase
...
int Command_bat (char **args, const Redirect *Inputs, const Redirect *Outputs, int *states) {	// fprintf(stderr, "mybat");
	...
	char fullpath_batchfile[MAX_PATH];	// fullpath of pervious batch file, e.g execute myshell test.bat, test.bat include the 							// line: myshell b.bat
						// fullpath_batchfile records the directory of test.bat ｼ景n case of dead circle, e.g excute 						// myshell test.bat, test.bat includes the line: myshell test.bat
	pid_t newpid;
	if (isbat)	// if executes from batchfile
		fprintf(stderr, "Line %d of inputfile \"%s\": ", bat_num, batchfile);
	...
	if (args[1]) {	// e.g.,  myshell test.bat    myshell < a.bxt
		...
		if (!output_num)
			Command_shell(inputfile, NULL, states);	// read command lines from Command_shell() ｼ径nalyse and execute ｼ継ut 									// the result to OutPuts
		else {
			for (i = 0; i < states[2]; i++) {
